...
```
在检测到键盘操作后, 实时更新`volume`变量, 并应用到mixer上.
```c
...
long min, max;
snd_mixer_selem_get_playback_volume_range(elem, &min, &max); // 获取mixer音量最大最小值
long set_volume = min + (volume * (max - min) / 100); // 将volume调整至合理范围
snd_mixer_selem_set_playback_volume_all(elem, set_volume); // 设置volume
```
