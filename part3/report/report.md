...
* `read_sample`: 将interleaved PCM数据(16/24/32位)转换为float, 便于后续处理.
* `write_sample`: 将float数据转换回PCM格式, 写入输出缓冲.
...
* WSOLA的核心是"重叠-相似拼接", 通过寻找最佳重叠点, 拼接时减少音质失真.
* 变速时, 输出帧的间隔根据`speed_ratio`调整, 实现变速不变调.
* 支持多通道和多种采样位宽.
...
均衡器提供了四种预设模式, 用户可以通过按键实时切换这些预设, 也可以用`e`键开关整个均衡器功能.
- **平衡模式**: 所有频段增益为0dB.
- **低音增强模式**: 对低频段施加16dB增益, 高频段衰减16dB.
- **高音增强模式**: 与低音增强模式相反.
- **人声增强模式**: 突出中频范围并抑制两端频段.
...
### 单片机算力不足
由于单片机算力有限, 最初实现的WSOLA算法在虚拟机中运行良好, 但是在单片机上计算较慢. 由于WSOLA算法与声卡写入过程为串行关系, 因此单片机上的倍速过程会导致音频明显不连续. 为此我们精简了最初实现的WSOLA算法, 并且使用`-O3`和`-ffast-math`加速.
